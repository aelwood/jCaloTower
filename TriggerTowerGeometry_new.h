...
class TriggerTowerGeometry
{
  ...
  public:
    ...
    double phiFromGct(const int& gPhi){//Returns the real value of phi from the gct iPhi
      double phi_degrees=0.0;
      if(gPhi <= 9) { //i.e. 1-36
        phi_degrees = (20.0*(double)gPhi);
      } else { //i.e. 37 - 72
        phi_degrees = -1.0 * ((18 - (double)gPhi) * 20.0) ;
      }
    ...
    double etaFromGct(const int& gEta){//Returns the real value of phi from the gct iPhi
      //Convert to iEta and then use normal method
      int ieta;
      if(gEta<11){
        ieta = (gEta-10)*4-2;
      }else{
